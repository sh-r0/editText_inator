...
void renderer_t::createFontTexture() {
    textureImage.resize(textureImage.size() + 1);
    textureImageMemory.resize(textureImage.size());
    textureImageView.resize(textureImage.size());
    textureSampler.resize(textureImage.size());
    ...
    createImage(tex_width, tex_height, VK_FORMAT_R8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage.back(), textureImageMemory.back());

    transitionImageLayout(textureImage.back(), VK_FORMAT_R8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(stagingBuffer, textureImage.back(), static_cast<uint32_t>(tex_width), static_cast<uint32_t>(tex_height));
    transitionImageLayout(textureImage.back(), VK_FORMAT_R8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    vkDestroyBuffer(device, stagingBuffer, nullptr);
    vkFreeMemory(device, stagingBufferMemory, nullptr);

    textureImageView.back() = createImageView(textureImage.back(), VK_FORMAT_R8_SRGB);
    ...
    for (uint32_t i = 0; i < textureImage.size(); i++)
...
void renderer_t::createDescriptorSets(void) {
    ...
    for (size_t i = 0; i < maxFramesInFlight_c; i++) {
        ...
        if (textureImageView.front() == VK_NULL_HANDLE) printf("%u is null handle\n", i);
        imageInfo.imageView = textureImageView.front();
        imageInfo.sampler = textureSampler.front();
...
void renderer_t::recordCommandBuffer(VkCommandBuffer _commandBuffer, uint32_t _imageIndex) {
    ...
    VkClearValue clearColor = { {{0,0,0, 0.0f}} };
...
inline void renderer_t::pushQuad(glm::vec2 _pos, glm::vec2 _size, glm::vec2 _texPos, glm::vec2 _texSize) {
    vertex_t v {};
    v.pos = _pos;
    v.texCoord = _texPos;
...
void renderer_t::pushText(glm::vec2 _pos, const std::string& _str) {
    for (char c : _str) {
        //if (c == '\n') {
        //    _pos.y += 1.5f * fontSize_c;
        //    _pos.x = 0;
        //    continue;
        //}
        if (c == char(219)) { //cursor marker val
            pushQuad({ _pos.x, _pos.y-fontSize_c },
                {1, 1.25f*fontSize_c},
                {0,0}, 
                {0,0});
            continue;
